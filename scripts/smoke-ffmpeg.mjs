...
run(process.env.FFMPEG_PATH || "ffmpeg", ["-y", "-f", "lavfi", "-i", "testsrc=size=320x180:rate=24:duration=1", "-c:v", "libx264", "-pix_fmt", "yuv420p", "-an", clipA]);
run(process.env.FFMPEG_PATH || "ffmpeg", ["-y", "-f", "lavfi", "-i", "testsrc2=size=320x180:rate=24:duration=1", "-c:v", "libx264", "-pix_fmt", "yuv420p", "-an", clipB]);
...
const probeText = run(process.env.FFPROBE_PATH || "ffprobe", ["-v", "error", "-show_entries", "format=duration,size", "-show_entries", "stream=codec_name,width,height", "-of", "json", output]);
...
console.log(JSON.stringify({ ok: true, output, duration, codec: video.codec_name, width: video.width, height: video.height, size: Number(probe.format?.size || 0) }, null, 2));
