...
export class ProviderRouter {
  ...
  get(id?: string): MediaProvider { const requested = id || process.env.CEO_MEDIA_PROVIDER || "mock"; const provider = this.providers.get(requested); if (!provider) throw new Error(`Unknown provider: ${requested}`); return provider; }
  async route(capability: MediaCapability, requested?: string): Promise<MediaProvider> {
    ...
    const primary = this.get(configured); const ph = await primary.health(); if (ph.ready && ph.capabilities.includes(capability)) return primary;
    const fallback = this.get(process.env.CEO_MEDIA_PROVIDER_FALLBACK || "mock"); const fh = await fallback.health(); if (fh.ready && fh.capabilities.includes(capability)) return fallback;
