...
export type StudioProvider = "gemini" | "mock" | "flow-web" | "ai-studio-web";
...
export class StudioRouter {
  async resolve(capability: StudioCapability, preference: ProviderPreference = "auto"): Promise<StudioRoute> {
    ...
    if (capability === "image") {
      if (enabled("ai-studio")) return { capability, preference, provider: "ai-studio-web", mode: "browser", ready: true, requiresBrowser: true, url: AI_STUDIO_URL, reason: "AUTO selected AI Studio Web because Gemini API key is unavailable" };
      ...
      if (enabled("flow")) return { capability, preference, provider: "flow-web", mode: "browser", ready: true, requiresBrowser: true, url: FLOW_URL, reason: "AUTO selected Flow Web because Gemini API key is unavailable" };
    ...
    return { capability, preference, provider: "gemini", mode: "api", ready: false, reason: "No real media route is available: configure Gemini API or enable Flow/AI Studio Web" };
  }

  async status() {
    const [autoImage, autoVideo, geminiImage, geminiVideo] = await Promise.all([
      this.resolve("image", "auto"), this.resolve("video", "auto"), this.resolve("image", "gemini"), this.resolve("video", "gemini")
    ]);
    return {
