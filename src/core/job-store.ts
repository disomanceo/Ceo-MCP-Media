...
export class JobStore {
  ...
  async due(limit = 20): Promise<MediaJob[]> { const now = Date.now(); return (await this.list()).filter((j) => ["queued", "waiting"].includes(j.status)).filter((j) => !j.nextRunAt || new Date(j.nextRunAt).getTime() <= now).sort((a, b) => a.createdAt.localeCompare(b.createdAt)).slice(0, limit); }
}
