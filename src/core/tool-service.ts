...
export const TOOL_NAMES = [
  "media.movie.create","media.movie.status","media.preflight.check",
...
export class MediaToolService {
  ...
  async call(name: string, args: any = {}) {
    switch (name) {
      case "media.preflight.check":
        return preflightPrompt(args.prompt, args.autoRewrite !== false);
      ...
      case "media.movie.create": {
        ...
        const storyboard = planStoryboard({
          ...
          brief: checked.safePrompt,
          totalDurationSec: args.totalDurationSec ?? 8,
        ...
        const job = await this.jobs.create({
          ...
          input: {
            ...
            anchorPrompt: args.anchorPrompt,
            shotPrompts: args.shotPrompts,
            dialogues: args.dialogues,
            outputPath: args.outputPath,
            subtitlePath: args.subtitlePath,
            compose: args.compose !== false,
            autoRewriteGuardrails: args.autoRewriteGuardrails !== false,
            videoConcurrency: args.videoConcurrency ?? 1,
            idempotencyKey: args.idempotencyKey
          }
        });
        return { movieJobId: job.id, projectId: project.id, status: job.status, preflight: checked, storyboard };
      }
      ...
      case "media.storyboard.plan": {
        ...
        await this.projects.setStoryboard(p.id, b);
        return b;
      }
      case "media.image.generate": {
        ...
        return this.jobs.create({ type: "image.generate", projectId: p?.id, provider: args.provider, idempotencyKey: args.idempotencyKey, input: { prompt: checked.safePrompt, outputPath, aspectRatio: args.aspectRatio ?? p?.aspectRatio, referenceImages: args.referenceImages ?? [] } });
      }
      case "media.video.generate": {
        const p = args.projectId ? await this.projects.get(args.projectId) : undefined;
        const outputPath = args.outputPath ?? path.join(dataDir(), "assets", `${safeName(args.name ?? "video")}-${Date.now()}.mp4`);
        const checked = preflightPrompt(args.prompt, args.autoRewriteGuardrails === true);
        return this.jobs.create({ type: "video.generate", projectId: p?.id, provider: args.provider, idempotencyKey: args.idempotencyKey, input: { prompt: checked.safePrompt, outputPath, aspectRatio: args.aspectRatio ?? p?.aspectRatio ?? "16:9", resolution: args.resolution ?? p?.resolution ?? "1080p", durationSec: Math.min(8, args.durationSec ?? 8), referenceImages: (args.referenceImages ?? []).slice(0, 3), firstFrame: args.firstFrame, lastFrame: args.lastFrame } });
      }
      case "media.video.regenerate": {
        const old = await this.jobs.get(args.jobId);
        if (old.type !== "video.generate") throw new Error("job is not a video generation job");
        const input = old.input as any;
        return this.jobs.create({ type: old.type, projectId: old.projectId, provider: args.provider ?? old.provider, idempotencyKey: args.idempotencyKey, input: { ...input, prompt: args.prompt ?? input.prompt, outputPath: args.outputPath ?? String(input.outputPath).replace(/\.mp4$/i, `-retry-${Date.now()}.mp4`) } });
      }
      ...
      case "media.director.review": {
        const p = await this.projects.get(args.projectId);
        if (!p.storyboard) throw new Error("project has no storyboard");
      ...
      case "media.flow.handoff": {
        const p = await this.projects.get(args.projectId);
        if (!p.storyboard) throw new Error("project has no storyboard");
        const handoff = this.flow.prepare(p.id, p.name, p.storyboard);
        const outputPath = args.outputPath ?? path.join(dataDir(), "assets", `${safeName(p.name)}-flow-handoff.json`);
        await writeJsonAtomic(outputPath, handoff);
        return { outputPath, handoff };
      }
      case "media.capabilities": return { versions: ["v1","v2","v3","v4","v5","v6","v7"], durable: true, autoMoviePipeline: true, autoWorker: process.env.CEO_MEDIA_AUTO_WORKER !== "false", providerAgnostic: true, flowOptional: true, guardrailPreflight: true, staleRunningRecovery: true, maxReferenceImages: 3, maxShotSec: 8, audioJobs: ["voice","music"], tools: TOOL_NAMES };
