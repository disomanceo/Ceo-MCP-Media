...
const descriptions: Record<string, string> = {
  "media.movie.create": "Create a durable movie workflow. provider=auto uses Gemini API when configured, otherwise browser-assisted Google AI Studio for anchor images and Google Flow for video shots; CapCut or FFmpeg can finalize.",
  "media.movie.status": "Read durable movie status, outputs and the next pending browser/CapCut external action when one exists.",
  "media.preflight.check": "Check a prompt for likely third-party/guardrail risk and optionally rewrite it before generation.",
  "media.studio.status": "Read V8 Studio Router choices for Gemini API, Google Flow Web and Google AI Studio Web.",
  ...
  "media.provider.status": "Read executable provider and Studio Router readiness without generating media.", "media.flow.handoff": "Prepare a portable Google Flow handoff package.", "media.capabilities": "Return V1-V8 capability manifest."
};

async function handle(req: RpcRequest) {
  if (req.method === "initialize") return { protocolVersion: req.params?.protocolVersion ?? "2025-06-18", capabilities: { tools: { listChanged: false } }, serverInfo: { name: "ceo-mcp-media", version: "1.2.0" } };
